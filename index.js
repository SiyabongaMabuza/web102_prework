...
const numContributions = GAMES_JSON.reduce((acc, contribution) => {
...
const amountRaised = GAMES_JSON.reduce((acc, amount) => {
...
const numGames = GAMES_JSON.reduce((acc, games) => {
    return acc + 1;
}, 0);
...
const unfundedString = `The number of unfunded games is ${numUnfundedGames > 0 ? numUnfundedGames : "0"}.`;
const raisedString = `We currently have ${numGames > 0 ? numGames : "0"} games. The total amount which has been raised for our games is $${amountRaised > 0 ? amountRaised : "0"}.`;

// create a new DOM element containing the template string and append it to the description container
const unfundedElement = document.createElement('p');
const raisedElement = document.createElement('p');
unfundedElement.textContent = unfundedString;
raisedElement.textContent = raisedString;
descriptionContainer.appendChild(raisedElement);
